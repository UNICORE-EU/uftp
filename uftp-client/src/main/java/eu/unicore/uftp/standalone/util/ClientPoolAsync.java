...
public class ClientPoolAsync implements Closeable {

	private static final Logger logger = Log.getLogger(Log.CLIENT, ClientPoolAsync.class);
	...
	final AsyncDownloader asyncDownloader;
	
	public ClientPoolAsync(int poolSize, ClientFacade clientFacade, String uri, boolean verbose, long splitThreshold) {
		...
		es = Executors.newFixedThreadPool(poolSize, new ThreadFactory() {
			...
			public Thread newThread(Runnable r) {
				...
				try{
					...
					if(reuseClients) {
						client = ClientPoolAsync.this.clientFacade.doConnect(ClientPoolAsync.this.uri);
						clients.add(client);
					}
		...
		Callable<Boolean> r = new Callable<>(){
			public Boolean call() throws Exception {
				...
				try(RandomAccessFile raf = new RandomAccessFile(file, "rw"))
				{
					...
					Pair<SocketChannel, Long> p = sc.prepareAsyncGet(remotePath, offset, size);
					asyncDownloader.add(p.getM1(), raf.getChannel(), p.getM2(), sc);
